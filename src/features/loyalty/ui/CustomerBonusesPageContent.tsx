      ...
      if (!result.ok) {
        setReorderError(
          result.error.code === "NO_AVAILABLE_PRODUCTS"
            ? reorderCopy.reorderUnavailable
    ...
    <>
                  ...
                  <li key={row.id} className="py-4 first:pt-0 last:pb-0">
                    {canOpenOrder ? (
                      <button
                        ...
                        className="flex w-full items-start justify-between gap-3 rounded-[12px] text-left transition-colors hover:bg-gray-50/80 active:bg-gray-50"
                      >
                        {body}
                      </button>
                    ) : (
      ...
      <AdminDictionaryProvider dictionary={adminDictionary}>
        <OrderDetailsDrawer
          ...
          isLoading={isPending}
          reorder={{
            label: reorderCopy.reorder,
            pendingLabel: reorderCopy.reordering,
