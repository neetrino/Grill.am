...
/** Saves the cover image for a popup via object storage. */
export async function persistPopupImage(
  ...
  const existing = await db
    ...
    .from(mediaAssets)
    .where(
      and(eq(mediaAssets.popupId, popupId), eq(mediaAssets.role, "COVER")),
    );
...
/** Removes popup cover media and deletes the stored object. */
