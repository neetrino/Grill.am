/**
 * Interactive CLI to create (or promote) an OPERATOR user.
 *
 * Operators can manage `/admin/orders` only (status, payment, notes, archive).
 *
 * Usage:
 *   pnpm create:operator
 *
 * Optional non-interactive flags:
 *   pnpm create:operator -- --email operator@example.com --password 'Secret1!'
 *
...
async function main(): Promise<void> {
  ...
  console.info("Create operator user (login = email)");

  const emailRaw = cli.email ?? (await askVisible("Operator email: "));
  ...
  const firstName =
    cli.firstName ?? (await askVisible("First name [Operator]: ", "Operator"));
  const lastName =
